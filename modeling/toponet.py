...
class BilinearSampler(nn.Module):
    ...
    def forward(self, feature_maps, sample_points, coord_format=None):
        ...
        # [FIX so với gốc] Bản gốc (sam_road/model.py) chỉ nhận 2 tham số và luôn
        # chuẩn hóa cứng theo `config.PATCH_SIZE` (vì SAM ViT dùng ảnh input vuông
        # cố định 1024x1024). Kiến trúc CoANet+TopoNet của bạn dùng feature map với
        # H, W thay đổi theo crop_size của dataset (không cố định 1024), nên phải
        # chuẩn hóa theo đúng (H, W) thực tế của feature_maps, không phải PATCH_SIZE
        # cố định -- nếu không mọi điểm sample sẽ bị lệch tọa độ khi crop_size != 1024.
        Returns:
...
class TopoNet(nn.Module):
    ...
    def forward(self, points, point_features, pairs, pairs_valid):
        ...
        # [FIX so với gốc] Bản gốc: torch.arange(batch_size) không có device=...
        # -> tensor mặc định nằm trên CPU trong khi points/pairs nằm trên GPU khi
        # train với CUDA/DataParallel -> RuntimeError: indices should be either on
        ...
        # [FIX so với gốc] Bản gốc dùng "if ... / if ... else ..." (2 khối if độc lập)
        # thay vì if/elif/else -> khi TOPONET_VERSION == 'no_tgt_features', pair_features
        # được gán đúng ở khối if đầu, nhưng NGAY SAU ĐÓ bị khối if/else thứ hai ghi đè
        # trở lại thành pair_features đầy đủ (vì 'no_tgt_features' != 'no_offset' nên
        # rơi vào else) -> ablation 'no_tgt_features' của bản gốc không hoạt động đúng
        # như thiết kế. Sửa thành if/elif/else để mỗi TOPONET_VERSION chỉ khớp đúng 1 nhánh.
        ...
        # [FIX so với gốc] Bản gốc dùng torch.logical_or(pairs_valid, all_invalid_pair_mask)
        # với all_invalid_pair_mask có shape [.., 1] -> broadcast biến TẤT CẢ n_pairs của
        # sample toàn-invalid thành "valid", khiến toàn bộ dữ liệu rác/padding của sample đó
        # được Transformer coi là dữ liệu thật. Bản sửa chỉ mở valid ở đúng 1 vị trí (index 0)
        # -> vẫn tránh được NaN do softmax trên toàn bộ key bị mask hết, nhưng không "giả vờ"
        # cả sample rác là dữ liệu hợp lệ.
